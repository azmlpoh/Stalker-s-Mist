...
#define GLITCH_THR 0.03
#define GLITCH_RECT_DIVISION 50.0
#define GLITCH_RECT_ITR 1
...
vec3 pattern2(in vec2 uv, in vec3 g) { return g.z<0. ? tex(uv).rgb : tex(uv+vec2(0.1*(g.z*2.0-1.0), 0.0)).rgb; }
...
vec3 pattern5(in vec2 uv, in vec3 g) { return g.z<0. ? tex(uv).rgb : vec3(tex(uv+vec2(RGB_SHIFT.r, 0.0)).r, tex(uv+vec2(RGB_SHIFT.g, 0.0)).g, tex(uv+vec2(RGB_SHIFT.b, 0.0)).b); }
